...
class EnhancedLongTermSelector:
    ...
    def generate_report(self, stocks: List[Dict]) -> str:
        ...
        for i, stock in enumerate(stocks, 1):
            ...
            report.append(f"   评分: {stock['score']:.1f} ({stock['rating']})")
            report.append(f"   价格: ¥{stock['price']:.2f} ({stock['change_pct']:+.2f}%)")

            fund = stock['details']['fundamental']
            report.append(
                f"   基本面: ROE={fund['roe']:.1f}% | 利润增长={fund['profit_growth']:+.1f}% | 股息率={fund['dividend_yield']:.2f}%"
            )

            val = stock['details']['valuation']
            if val['peg']:
                report.append(f"   估值: PE={val['pe']:.1f} | PEG={val['peg']:.2f} ({val['level']})")

            sig = stock['signal']
            report.append(f"   信号: {sig['decision']} (得分{sig['score']:.1f}, {sig['signal_count']}项满足)")
            report.append('')
...
if __name__ == '__main__':
    ...
    if top_stocks:
        ...
        report_file = recommend_dir / f'enhanced_long_term_recommendation_{date_suffix}.txt'
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(report)
        selector.save_selected_watchlist(top_stocks, 'watchlist_long_advanced_long_term.json')
