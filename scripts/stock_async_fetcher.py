...
class StockAsyncFetcher:
    ...
    def fetch_fund_flow(self, code: str, use_cache: bool = True, max_age_hours: int = 8) -> Dict:
        """
        获取资金流向
        ...
        """
        if use_cache:
            cached = self.cache.get_fund_flow(code, max_age_hours=max_age_hours)
            if cached:
                return {
                    'main_in': cached['main_in'],
                    'retail_in': cached['retail_in'],
                    'main_ratio': cached['main_ratio'],
                    'source': 'cache',
                }
        ...
        if last_error:
            print(f"获取{code}资金流失败: {last_error}")
        try:
            time.sleep(0.05)
        except Exception as e:
            pass
